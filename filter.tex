...
\)@
\end{lstlisting}
\item \emph{Decomposition}. The program can be decomposed into an
...
\rangle^{\GasAll}
\)@\end{lstlisting}
\item \emph{Action selection}. The selected action is \(\ActStep\) in this case.
\item \emph{Residue Removal}. In this case, the residue removal process doesn't do anything.
\item \emph{User select}. Since the stepper selected the action
  \(\ActStep\), it will stop here and yield its control to user.
\item \emph{Instruction Transition}. The selected expression will be instruction-transitioned to
\begin{lstlisting}[language=hazel]
if 3 == 1 then
  1
else
  fib(3 - 1) + fib(3 - 2)\end{lstlisting}
