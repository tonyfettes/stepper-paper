...
a mutated of the term of untyped lambda calculus. Apart from
constructs like variables, applications, and lambdas, it also includes
some new constructs, a forall pattern \(\PatExpr\), a value pattern
\(\PatValue\). The action is a tag indicating the action to be taken
when the pattern matches an expression. It has two form, \(\ActSkip\)
and \(\ActStep\). The gas is another tag indicating the live time of
the effect of a filter. It has two form, \(\GasOne\) and
\(\GasAll\). However, the tuple form of the filter expression is not
intuitive for user to construct and typewrite in the Hazel
editor. Therefore, we added some syntax sugar to help user better
...
\begin{table}[htpb]
  ...
  \end{tabular}
  \caption{Mapping between the syntax sugar used in the Hazel editor and the actual syntax representation of filter expressions.}\label{tbl:filter-syntax-sugar}
\end{table}
...
``microscopic'' steps. Here is a detailed description of first a few
...
\item \emph{Instrumentation}. \label{num:simple_example_instrument} In this
  ...
  The matching process itself is a recurisve process as well. If a
  ...
  most of the cases, we want the filter pattern to be able to match
  ...
  \begin{enumerate}
  \item If \(f\) is wildcard \(\$e\), exits the process and returns
    that \(e\) and \(\$e\) matches.
  \item If \(f\) is wildcard \(\$v\), and \(e\) is a value, exits the process and returns
    that \(e\) matches \(\$v\).
  \item If \(f\) is wildcard \(\$v\), and \(e\) is not a value, exits
    the process and returns that \(e\) does not match \(\$v\).
  \item If the \(e\) and \(f\) does not share a common construction,
    exits the matching process and returns \(e\) and \(f\) does not
    match.
  \item Otherwise, iterate over each pair of child expressions or
    child filters in \(e\) and \(f\). Since \(e\) and \(f\) shares the
    same construction, their number of child items are always the
    same. For each pair, goes to step 1.
  \item If every pairs in step 2 matches, exit the process and returns that \(e\) and \(f\) matches.
  \end{enumerate}
  ...
  Before instrumentation, we will assign priorities to active
  filters. The priority is assign in the way they are being added to
  the list active filters. Here, we assign the priority 0 to the first
  filter, 1 to the second one, and 2 to the last one. The priority
  will be used when decising which action to take for the next step.
  ...
  inner-most sub-expressions, like \lstinline[language=hazel]{1},
  \lstinline[language=hazel]{2}, etc., we do not instrument them since
  they are value. Because a value cannot be evaluated, it would make
...
\item \emph{Making Decision}. \label{num:simple_example_select} Decide
  which action (stop or continue) to take base on all the residue
  expressions around mark in the evaluation context. The action is
  decided using the following set of informal rules:

  \begin{enumerate}
  \item For a expression that is not a residue expression, the action
    will be the one that comes from the sub-expression that contains
    the mark.
  \item For a residue expression, if the action comes from its only
...
\item \emph{Residue removal (Decaying)}. Residue expressions that
  has gas of one is removed from the expression. This step is
