...
dynamics of the language. Finally, we discuss the optional part of the
semantics, the statics of the language, and show that the calculus satisfies
...
The syntax of the filtered stepper calculus (Fig.~\ref{fig:filter-syntax}) is an
extended version of the untyped lambda calculus. Additionally, natural numbers
and addition are included for a more concrete demonstration. It introduces two
new expressions to the untyped lambda calculus, \texttt{Filter} and
\texttt{Residue}. The \texttt{Filter} expression is visible at the user level,
indicating that the enclosed expression is filtered. The \texttt{Residue}
expression is used internally by the stepper and is not visible to the user. It
signals the \emph{residue} of an effect of applying a filter to an expression.

A filter is a tuple of a pattern, an action, and a gas. The pattern is
a variant of the term in the untyped lambda calculus. Apart from
constructs like variables, applications, and lambdas, it also includes
some new constructs: a forall pattern \(\PatExpr\) and a value pattern
\(\PatValue\). The action is a tag indicating the action to be taken
when the pattern matches an expression. It has two forms, \(\ActSkip\)
and \(\ActStep\). The gas is another tag indicating the lifetime of
the effect of a filter. It has two forms, \(\GasOne\) and
\(\GasAll\). However, the tuple form of the filter expression is not
intuitive for users to construct or type in the Hazel
editor. Therefore, we added syntactic sugar to help users better
understand and write the filter expression, as shown in
Table~\ref{tbl:filter-syntax-sugar}.

\begin{table}[htpb]
  \begin{tabular}{ll}
    Hazel code with sugar & Actual syntax representation \\
  ...
  \end{tabular}
  \caption{Mapping between the syntactic sugar used in the Hazel editor and the actual syntax representation of filter expressions.}\label{tbl:filter-syntax-sugar}
\end{table}

The residue expression \(\Residue{a}{g}{l}{e}\) is an expression that is used to
store the residue of the effect of a filter. It has four components: an action
\(a\), a gas \(g\), a priority \(l\), and an expression \(e\). The action and the
...
statement is not possible within the current pattern definition in
...
\lstinline[language=hazel]{3}, then \lstinline[language=hazel]{3 + 3}
becomes \lstinline[language=hazel]{6}, and finally
\lstinline[language=hazel]{6 + 4} becomes
...
\item \emph{Instrumentation}. \label{num:simple_example_instrument} In this
  ...
  wildcard matchers: \lstinline[language=hazel]{$e} and
  \lstinline[language=hazel]{$v}. The former one matches against any
  expression, while the latter one matches against any value. Assuming
  we are checking if the expression \(e\) matches the filter
  \(f\). This process can be informally concluded as the following
  steps.

  \begin{enumerate}
  \item If \(f\) is wildcard \(\$e\), exit the process and return
    that \(e\) and \(\$e\) match.
  \item If \(f\) is wildcard \(\$v\), and \(e\) is a value, exit the process and return
    that \(e\) matches \(\$v\).
  \item If \(f\) is wildcard \(\$v\), and \(e\) is not a value, exit
  ...
  \item If every pair in step 2 matches, exit the process and return that \(e\) and \(f\) match.
  \end{enumerate}
  ...
  innermost sub-expressions, like \lstinline[language=hazel]{1},
  \lstinline[language=hazel]{2}, etc., we do not instrument them since
  they are values. Because a value cannot be evaluated, it would make
  no sense if we allow the stepper to stop before ``evaluating'' an
  value.

  During the instrumentation of the second filter, the sub-expression
  \lstinline[language=hazel]{1 + 2 + 3 + 4} matches the pattern, so we
...
\item \emph{Making Decision}. \label{num:simple_example_select} Decide
...
\item \emph{Residue removal (Decaying)}. Residue expressions that
  have gas of one are removed from the expression. This step is
...
\item \emph{User Interaction}. If the selected expression produced in
  ...
  will skip over this step and go into the next step. (S-Filter and
  ...
  will skip over this step and go into the next step.
...
The motivating examples above have demonstrated briefly how the filter
...
\fbox{\(\JustStep{e}{e'}{n}\)} Expression \(e\) takes \(n\) steps to \(e'\), completely ignoring the behavior of the filters and residues.
\begin{mathpar}
  \inferrule[J-Step]{
...
We want the (Typed) Filtered Stepper Calculus to conform to the most
...
Optionally, we want to give the filtered stepper calculus a static semantics so that it can have reasonable behavior with regard to free variables. The most notable benefit we can obtain from a typing system is that we can enforce that there are no free variables in the pattern and the expression before we test whether they match.

We can extend the definition of the filtered stepper calculus to include types.
\begin{figure}[h]
  \begin{equation*}
    \begin{array}{rcl}
      \ldots       &         & \ldots \\
